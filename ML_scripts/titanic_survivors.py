...
path_download = os.path.join(os.path.dirname(os.path.dirname(os.path.realpath(__file__))),
                             'helpers', 'competition_downloader.py')
...
y_train = pd.read_csv(os.path.join(path_out, all_files[2]), index_col='PassengerId',
...
def fill_age(cols, fill):
    ...
    :param fill: data to be inserted to particular missing fields
...
y_output['Survived'] = y_pred

os.mkdir(os.path.join(path_out, 'output'))
y_output.to_csv(path_or_buf=os.path.join(path_out, 'output', 'prediction_submission.csv'),)
